...
class MainActivity : AppCompatActivity() {
  ...
  private fun postPermissionsSetup() {
    try {
      setupAudioEngine()
      // Audio engine pehle se start karo — taake mic click pe immediately record ho
  ...
  private fun setupGeminiLive() {
    try {
      ...
      geminiLive.onSetupComplete = {
        runOnUiThread {
          try {
            // Audio already started in postPermissionsSetup — bas status update karo
            if (::audioEngine.isInitialized && !audioEngine.isRunning()) {
              audioEngine.start(this@MainActivity)
            }
  ...
  private fun setupClickListeners() {
    micButton.setOnClickListener {
      try {
        ...
        // Pehle ensure audio engine chal rahi hai
        if (::audioEngine.isInitialized) {
          audioEngine.start(this@MainActivity)
        }

        // Gemini connected nahi hai? Try connect karo
        if (::geminiLive.isInitialized && !geminiLive.isConnected) {
          val apiKey = BuildConfig.GEMINI_API_KEY
          if (apiKey.isBlank() && (prefs.getString("api_key", "") ?: "").isBlank()) {
